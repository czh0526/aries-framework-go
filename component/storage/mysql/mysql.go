...
type dbEntry struct {
	...
	Tags  []spistorage.Tag `json:"tags,omitempty"`
}
...
func (p *Provider) OpenStore(name string) (spistorage.Store, error) {
	if name == "" {
...
func (p *Provider) SetStoreConfig(name string, config spistorage.StoreConfiguration) error {
	...
	if !ok {
		return spistorage.ErrStoreNotFound
	}
...
func (p *Provider) GetStoreConfig(name string) (spistorage.StoreConfiguration, error) {
	name = strings.ToLower(name)
	...
	openStore, ok := p.dbs[name]
	if !ok {
		return spistorage.StoreConfiguration{}, spistorage.ErrStoreNotFound
	}

	storeConfigBytes, err := openStore.Get(storeConfigKey)
	if err != nil {
		return spistorage.StoreConfiguration{},
			fmt.Errorf(`failed to get store configuration for "%s": %w`, name, err)
	}

	var storeConfig spistorage.StoreConfiguration

	err = json.Unmarshal(storeConfigBytes, &storeConfig)
	if err != nil {
		return spistorage.StoreConfiguration{}, fmt.Errorf("failed to unmarshal store configuration: %w", err)
	}
...
func (p *Provider) GetOpenStores() []spistorage.Store {
...
func (s *store) Put(key string, value []byte, tags ...spistorage.Tag) error {
...
func (s *store) GetTags(key string) ([]spistorage.Tag, error) {
...
func (s *store) Query(expression string, options ...spistorage.QueryOption) (spistorage.Iterator, error) {
...
func (s *store) Delete(k string) error {
	...
	if err != nil {
		return fmt.Errorf(spistorage.ErrDataNotFound.Error(), err)
	}
...
func (s *store) Batch(batch []spistorage.Operation) error { // nolint:gocyclo
	if len(batch) == 0 {
...
func (s *store) bulkAppendToQuery(b spistorage.Operation, query *string, values *[]interface{}) error {
	if len(b.Value) > 0 {
...
func (s *store) updateTagMap(key string, tags []spistorage.Tag) error {
	s.lock.Lock()
	defer s.lock.Unlock()
...
func (s *store) getTagMap(createIfDoesNotExist bool) (tagMapping, error) {
	...
	if err != nil {
		if createIfDoesNotExist && errors.Is(err, spistorage.ErrDataNotFound) {
...
func (s *store) getDBEntry(key string) (dbEntry, error) {
	...
	if err != nil {
		if strings.Contains(err.Error(), valueNotFoundErrMsgFromMySQL) {
			return dbEntry{}, spistorage.ErrDataNotFound
		}
...
func validatePutInput(key string, value []byte, tags []spistorage.Tag) error {
	if key == "" {
		return errors.New("key cannot be empty")
	}
...
func checkForUnsupportedQueryOptions(options []spistorage.QueryOption) error {
...
func getQueryOptions(options []spistorage.QueryOption) spistorage.QueryOptions {
	var queryOptions spistorage.QueryOptions
