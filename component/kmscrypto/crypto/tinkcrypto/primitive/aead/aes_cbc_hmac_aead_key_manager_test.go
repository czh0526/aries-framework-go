...
func TestEncryptDecrypt(t *testing.T) {
	plaintext := []byte("this is a message for testing")
	aad := []byte("this is a data for additional encrypt")
	...
	for _, test := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			ciphertext, err := tinkAEAD.Encrypt(tc.plaintext, tc.aad)
			require.NoError(t, err)

			out, err := tinkAEAD.Decrypt(ciphertext, aad)
...
func TestPrimitiveIETFTestVector(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			ct := make([]byte, len(tc.nonce)+len(tc.ciphertext)+len(tc.authTag))
			copy(ct, tc.nonce)
			copy(ct[len(tc.nonce):], tc.ciphertext)
			copy(ct[len(tc.nonce)+len(tc.ciphertext):], tc.authTag)

			out, err := tinkAEAD.Decrypt(ct, tc.aad)
			require.NoError(t, err, "unable to decrypt")
			require.EqualValues(t, tc.plaintext, out)
		})
	}
}
