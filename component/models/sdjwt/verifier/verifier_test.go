...
func TestHolderVerification(t *testing.T) {
	...
	combinedFormatForIssurance, err := token.Serialize(false)
	require.NoError(t, err)

	_, err = holder.Parse(combinedFormatForIssurance, holder.WithSignatureVerifier(verifier))
	require.NoError(t, err)

	holderSigner := modeljwt.NewEd25519Signer(holderPrivateKey)

	cfi := common.ParseCombinedFormatForIssuance(combinedFormatForIssurance)
