...
func verifyKeyBindingJWT(holderJWT *modeljwt.JSONWebToken, pOpts *parseOpts) error {
	...
	if err != nil {
		return fmt.Errorf("mapstruct verifyHolder failed, error: %w", err)
	}

	if err = d.Decode(holderJWT.Payload); err != nil {
		return fmt.Errorf("mapstruct verifyHodlder decode failed, error: %w", err)
	}
